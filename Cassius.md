# Character Info - Cassius Vale
### Class: Paladin
* Level 3

###  Race: Human
* When you pray for guidance and ask *“What here is evil?”*, the GM answers honestly 

###  Alignment: Good
* Endanger yourself to protect someone weaker than you

### Look
* Eyes: **Glowing eyes**
* Hair: **Helmet (rarely removed)**
* Symbol: **Worn holy symbol (cracked sun disk)**
* Body: **Bulky body**

### Ability Scores
| Stat | Score | Modifier |
| ---- | ----- | -------- |
| STR  | 16    | +2       |
| CHA  | 16    | +2       |
| CON  | 13    | +1       |
| WIS  | 13    | +1       |
| DEX  | 9     | +0       |
| INT  | 8     | -1       |


### HP / Damage / Load
* **Damage:** d10
* **Max HP:** 25
* **Max Load:** 14

### Starting Gear
* Scale armor (2 armor, 3 weight)
* Longsword (+1 damage, 1 weight)
* Masterwork Shield (+1 armor, 0 weight)
* Holy symbol (0 weight)
* **Total armor:** 3
* **Total weight:** 4

## Class Moves
### Lay on Hands (CHA)
When you **touch someone, skin to skin, and pray for their well-being **, roll+CHA
* On a 10+ you heal 1d8 damage or remove one disease.
* On a 7–9, they are healed, but the damage or disease is transferred to you.
  * if healing yourself, you are healed but take -1 ongoing to lay on hands until you rest.

### Armored
Ignore the *clumsy* tag on armor 

### I Am the Law (CHA)
When you command with divine authority roll +CHA:
* 7+: They obey, flee, or attack
* 10+: +1 forward vs them
* 6-: they do as they please and you take -1 forward against them

### Voice of Authority
Take +1 to order hirelings.

### Staunch Defender
when you defend you always hold +1 even on a 6-

### Quest
You swear a sacred mission and gain divine boons and burdens 

**Quest:**
> Purge the temple of chaos from the borderlands

**Boons:**
* Eyes that see in darkness
* A freedom from hunger, thirst, and sleep

**Vows:**
* Valor (forbidden: suffering an evil creature to live)
* Honor (forbidden: cowardly tactics and tricks)

## Basic Moves
@https://exposit.github.io/dw-srd/dw_basic_moves.html

## Special Moves
@https://exposit.github.io/dw-srd/dw_special_moves.html

# World Info
## About Equipment
@https://exposit.github.io/dw-srd/dw_equipment.html
